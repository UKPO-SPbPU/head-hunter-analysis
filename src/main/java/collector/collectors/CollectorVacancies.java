...
public class CollectorVacancies {
    ...
    public void start(String startId, String endId, DBInfo dbInfo) {
        LOGGER.info("Загрузка вакансий начинается с id = " + startId);
        int start = Integer.parseInt(startId);
        int end = Integer.parseInt(endId);
        for (int i = 0;
             start + i <= end || start + (i - MAX_COUNT_FOR_ONE_DOWNLOAD) <= end;
             i += MAX_COUNT_FOR_ONE_DOWNLOAD) {
        ...
        LOGGER.info("Завершили загрузку вакансий в базу данных");
    }
